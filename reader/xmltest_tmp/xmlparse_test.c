...
uint32_t xml_util_get_num_nodes(
  xmlNodePtr pNodePtr, const char *nodeName)      
{
  uint32_t   i = 0;                               
...
xmlNodePtr xml_util_get_node(xmlNodePtr pNodePtr,
  ...
  for (curPtr = xmlFirstElementChild(pNodePtr);   
    curPtr; curPtr = xmlNextElementSibling(curPtr)) {    
      ...
      if (node_count > node_index)
        return xml_util_get_node(curPtr, nodeName,    
          node_index);
      else
...
int xml_util_get_radio_config(struct xmlConfigInfo *pXmlConfig)
{
	...
	RETURN_ON_FALSE(xml_util_get_node_data(
		docPtr, nodePtr, "Radio", key_value_pair, num_pairs, 0));
...
int xml_util_get_upper_config(struct xmlConfigInfo *pXmlConfig)
{
	...
	RETURN_ON_FALSE(xml_util_get_node_data(
		docPtr, nodePtr, "Upper", key_value_pair, num_pairs, 0));
...
int main()
{
	...
	ret = xml_util_load_file(config_xml_name, &docPtr, &rootPtr,
